...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                if can_contain(self.ant):
                    if contain_ant(self.ant)==None:
                        self.ant.ant = insect
                    else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.queen_num==1:
                return None
...
class Insect(object):
    ...
    def __init__(self, armor, place=None):
        ...
        self.effect_dict["STUN"] = 0
        self.effect_dict["SLOW"] = list()
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 5
        target_point = self.place
        target_range = 0

        while target_range < self.min_range:
            target_point = target_point.entrance
            target_range = target_range + 1

        while target_point.entrance != None \
              and target_point.entrance.name != 'Hive' \
              and len(target_point.bees) == 0\
              and target_range < self.max_range:
            target_point = target_point.entrance
            target_range = target_range + 1

        return random_or_none(target_point.bees)
        # END Problem 5
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if new_armor <= 0:
            temp_bees_list = list(self.place.bees)
...
class HungryAnt(Ant):
    ...
    food_cost = 4
    time_to_digest=3
    digesting=0  # Change to True to view in the GUI
    # END Problem 10
...
class BodyguardAnt(Ant):
    ...
    def __init__(self):
        Ant.__init__(self, 2)
        self.ant = None
         # The Ant hidden in this bodyguard
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        # BEGIN Problem 13
        Ant.__init__(self, armor=1)
        QueenAnt.number_of_queenant=QueenAnt.number_of_queenant+1
        self.queen_num=QueenAnt.number_of_queenant
        self.ant_behind=[]
        self.watersafe = True
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.queen_num==1:
            ...
            where=self.place.exit
            while where!=None:
                if where.ant!=None:
                    if where.ant.container == True and where.ant.ant!=None:
                        self.ant_behind=self.ant_behind+[where.ant.ant]
                    self.ant_behind=self.ant_behind+[where.ant]

                where=where.exit
            if  self.place.ant != self:
                self.ant_behind=self.ant_behind+[self.place.ant]
            if self.ant_behind!=[]:
                for ant in self.ant_behind:
                    if isinstance(ant, ThrowerAnt):
                        ant.damage = ThrowerAnt.damage * 2
                    elif isinstance(ant, FireAnt):
                        ant.damage = FireAnt.damage * 2
                    elif isinstance(ant, NinjaAnt):
                        ant.damage = NinjaAnt.damage * 2
                    elif isinstance(ant, TankAnt):
                        ant.damage = TankAnt.damage * 2
    ...
    def reduce_armor(self, amount):
        ...
        if self.queen_num==1 and self.armor==0:
            bees_win()
        # END Problem 13
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    def get_slow(bee, colony):
        if colony.time % 2 == 0:
            Bee.action(bee, colony)
            bee.effect_dict["SLOW"] = [sub_to_zero(num) for num in bee.effect_dict["SLOW"]]
        else:
            length_of_list = len(bee.effect_dict["SLOW"])
            bee.effect_dict["SLOW"][length_of_list - 1] = sub_to_zero(bee.effect_dict["SLOW"][length_of_list - 1])
        bee.effect_dict["SLOW"] = [num for num in bee.effect_dict["SLOW"] if num != 0]
    return get_slow, "SLOW"
...
def make_stun(action):
    ...
    """
    # BEGIN Problem EC
    def get_stun(bee, colony):
        bee.effect_dict["STUN"] = sub_to_zero(bee.effect_dict["STUN"])
    return get_stun, "STUN"
    # END Problem EC

def apply_effect(effect, bee, duration):
    """Apply a status effect to a BEE that lasts for DURATION turns."""
    # BEGIN Problem EC
    temp_effect, effect_name = effect(Bee.action)
    #bee.effect_dict[effect_name] = duration + bee.effect_dict[effect_name]
    if effect_name == "STUN":
        bee.effect_dict[effect_name] = duration + bee.effect_dict[effect_name]
    elif effect_name == "SLOW":
        bee.effect_dict[effect_name].append(3)

    def effect_action(colony):
        nonlocal temp_effect, effect_name
        temp_effect(bee, colony)
        if bee.effect_dict["STUN"] == 0 and len(bee.effect_dict["SLOW"]) == 0:
            bee.action = bee.original_action
        elif bee.effect_dict["STUN"] != 0:
            temp_effect, effect_name = make_stun(Bee.action)
        else:
            temp_effect, effect_name = make_slow(Bee.action)
